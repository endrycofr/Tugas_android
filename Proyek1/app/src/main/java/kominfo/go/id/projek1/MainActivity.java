...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Event handler for listView onClick
        listView.setOnItemClickListener((parent, view, position, id) -> {
            // Convert object to Map
            Map<String, Object> data = (Map<String, Object>) parent.getAdapter().getItem(position);
            // Activate InsertAndViewActivity through explicit intent with data
            Intent intent = new Intent(this, InsertAndViewActivity.class);
            intent.putExtra("filename", data.get("name").toString());
            startActivity(intent);
        });
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.miTambah) {
            // Activate InsertAndViewActivity via explicit intent - no data
            startActivity(new Intent(this, InsertAndViewActivity.class));
        }
        return super.onOptionsItemSelected(item);
    }
